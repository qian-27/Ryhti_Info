            ...
            if let errorMessage = errorMessage {
                ...
                List(buildings, id: \.id) { building in
                    VStack(alignment: .leading) {
                        ...
                        Text("\(building.properties.address_fin ?? "Unknown Street"), \(building.properties.address_number.map(String.init) ?? "Unknown Address Number"), \(building.properties.postal_office_fin ?? "Unknown Postal Code"), Helsinki")
                            .font(.headline)
                            .fontWeight(.bold)
                            .padding([.top, .bottom], 10)
