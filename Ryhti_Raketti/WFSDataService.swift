...
class WFSDataService {
    func fetchBuildings(streetName: String, completion: @escaping (Result<[Feature], Error>) -> Void) {
        // Normalize input and split into parts
        let trimmed = streetName.trimmingCharacters(in: .whitespacesAndNewlines)
        let parts = trimmed.split(separator: " ")

        // Build CQL: if last token is a number, match street+number exactly; otherwise street-only
        let cqlFilter: String
        if let last = parts.last, let num = Int(last) {
            let street = parts.dropLast().joined(separator: " ")
            // exact street + house number
            cqlFilter = "katunimi_suomi ILIKE '\(street)' AND osoitenumero=\(num)"
        } else {
            // all buildings on the street
            cqlFilter = "katunimi_suomi ILIKE '\(trimmed)'"
        }

        // Encode the CQL filter
